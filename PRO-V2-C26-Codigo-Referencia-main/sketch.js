...
function setup() {
  ...
  //3 carrega frames
  //4 carrega frames em loop

  var framesbarco=boatSpriteData.frames;

  for (var i=0;i<framesbarco.length;i++){

    var posi=framesbarco[i].position
    var img=imageframes.get(posi.x,posi.y,posi.w,posi.h)
...
function showBoats() {
  if (boats.length > 0) {
    ...
    var boat = new Boat(width, height - 60, 170, 170, -60,barcoanimation);
    boats.push(boat);
  }
}
